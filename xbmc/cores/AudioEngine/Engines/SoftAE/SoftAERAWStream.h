...
class CSoftAERAWStream : public ISoftAEStream
{
protected:
  friend class CSoftAE;

  CSoftAERAWStream(enum AEDataFormat format, unsigned int sampleRate, unsigned int encodedSamplerate, CAEChannelInfo channelLayout, unsigned int options);
  virtual ~CSoftAERAWStream();
...
public:
  ...
  virtual float             GetReplayGain   ()             { return 1.0f; }
  ...
  virtual double            GetResampleRatio() { return 1.0; }
  virtual bool              SetResampleRatio(double ratio) { return false; }
...
private:
  ...
  std::list<CAEBuffer*> m_outBuffer;
